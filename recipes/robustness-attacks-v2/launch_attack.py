...
def schedule_iteration(
	*,
	...
	step1_cmd = (
		"python recipes/robustness-attacks-v2/scripts/step1.py "
	...
	if eval_mode == "benchmark":
		# step 2 — inject distractors into benchmark files
		step2_cmd = (
			"python recipes/robustness-attacks-v2/scripts/step2.py "
		...
		pattern = "recipes/robustness-attacks-v2/prompts/eval-prompt-*.yaml"
		...
		for distractor_type in DISTRACTOR_TYPES:
			for position in positions:
				...
				prompt_template = f'/nemo_run/code/recipes/robustness-attacks-v2/prompts/eval-prompt-{position}.yaml'
				eval_expname = f"{iteration_expname}_eval_type-{distractor_type}_pos-{position}"
				step4_run_after.append(eval_expname)

				# step 3 — benchmark eval
				eval(
					ctx=wrap_arguments(
		...
		step3_judge_cmd = (
			"python recipes/robustness-attacks-v2/scripts/step3_judge.py "
			f"--output-folder {output_folder} "
			f"--iteration-id {current_iteration}"
		)
		if judge_model:
	...
	step4_cmd = (
		"python recipes/robustness-attacks-v2/scripts/step4.py "
		f"--output-folder {output_folder} "
		f"--iteration-id {current_iteration} "
