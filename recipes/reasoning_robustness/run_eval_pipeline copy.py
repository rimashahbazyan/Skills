...
def main():
    ...
    for noise in NoiseType:
        input_file = "nemo_skills/dataset/gpqa/diamond.jsonl"
        output_file = f"/workspace/Robustness-GPQA/datasets/gpqa-long/diamond_with_{noise.name.lower()}.jsonl"
        ...
        for prompt_id in [1, 2, 3]:
            run_eval_pipeline(
                prompt_id=prompt_id,
                dataset=f"diamond_with_{noise.name.lower()}",
